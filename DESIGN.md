...
- **1日の丸**: 1本の輪を、P・F・C の kcal で起点（上）から時計回りに色分けし、一周で1日の目安にする。色の間に細いすき間を空ける。帯の丸は 28・線 4.5、日のまとめの大きな丸は 128・線 14。Swift Charts の `SectorMark`（`innerRadius` と `angularInset`）で描く
