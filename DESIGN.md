...
- **待っている表示**: 画面全体をふさがず、待っているもののその場に出す。いつ出すかは `ios/AGENTS.md` の「待っている表示」
...
- Don't: カスタムフォントを使う
- Don't: 画面に「AI」「質問」「相談」という言葉を出す。推定であることは、推定の印と待っている表示で伝える（規約のために出す AI の返事の1行と、サインインの画面の同意は除く。`ios/AGENTS.md` の「会話」）
