...
【要確認】「ユーザー」は仮の名前。アプリの文脈に合った呼び方が決まったら置き換える。
...
| オブジェクト | 定義 | 属性（根拠） |
|---|---|---|
| ユーザー（新規・仮称） | nu-tori で自分の体づくりに取り組む人。Sign in with Apple のアカウントに1対1で対応する。管理画面を使う人（管理者）は含めない | 身長・生年月日・性別（目標_2、目安_1: 初期の消費量の見積もり）、活動量の初期値（目標_2: 歩数が取れないときの3択） |
...
| 相談（新規） | AI との1つの会話。初回リリースでは、1つの質問と1つの答えで終わる | 始めた日時（フィードバック_1,2）、プリセットから始めたか（フィードバック_2） |
| 発言（新規） | 相談の中の1回の発言。ユーザーの質問か、AI の答え | 話し手（ユーザーか AI か）・本文・日時（フィードバック_1,2） |
...
| ユーザー →〈相談〉→ 相談 | 1 → 0..n | 一覧ビュー＋空表示 | フィードバック_1,2（データとして必ず残す。一覧を画面に出すかは⑤で決める） |
| 相談 →〈発言〉→ 発言 | 1 → 1..n | 一覧ビュー（初回リリースは質問と答えの2件） | フィードバック_1,2 |
...
| 相談 | チャット、AI アドバイス | AI との1つの会話を指す |
| 発言 | メッセージ | 相談の中の1回の発言 |
| 管理者 | （ユーザーの一種として扱わない） | 管理画面を使う人は、ユーザーとは別の概念にする |
