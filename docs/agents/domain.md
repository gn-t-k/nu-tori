...
- **今の決定**：ルートの `AGENTS.md` の「情報の置き場」のとおり、関係するディレクトリの `AGENTS.md` にある。
- **`docs/adr/`**：決めた理由と検討した案を知りたいときに読む、当時の記録。ストックに書き起こしていない決定は、ここにもある（「情報の置き場」）。
...
出力が今の決定（`AGENTS.md`。書き起こしていないものは ADR）と矛盾する場合は、黙って上書きせずに明示する：

> _ADR-0007（注文のイベントソーシング）と矛盾するが、〜のため見直す価値がある_
