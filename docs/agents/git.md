...
Swift のコード・テスト・依存を変えたブランチは、PR を出す前と、PR に push する前に、好みのレビューを通す。新しい文脈のサブエージェントに次を頼み、外れた箇所が0件になるまで直す。直したら、新しい文脈のサブエージェントでもう一度照らす。

1. `docs/agents/coding-style.md` と `docs/agents/testing.md` を読む
2. `git fetch origin <マージ先のブランチ>` のあと、`git diff origin/<マージ先のブランチ>...HEAD` で差分を読む。判断に要るだけ、変更したファイルの前後や関係するファイルも読む
3. 差分で足した行と変えた行のすべてに、2つの文書のすべてのルールを当てる。差分の外の既存のコードは対象にしない
