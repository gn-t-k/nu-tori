# Turso をユーザーごとの DB に使う案の調査（Turso Cloud・Turso Sync・D1 と Durable Objects との比較）

調査日: 2026-09-25
対象: サーバーの DB に Cloudflare D1 ではなく Turso を使い、(1) ユーザーごとに DB を1つ持つ、(2) iPhone に Embedded Replicas か Turso Sync を置いてオフラインに備える、という案。前提は ADR-0001（設計思想の順位）、ADR-0008（記録の正本はサーバー、端末はキャッシュ）、ADR-0009（目標と目安の計算はサーバー、週ごとの見直しを定期実行）、`docs/research/server-platform.md`（実行基盤の比較と D1 の見積もり）。
...
- **月額の見積もり（LLM を除く、1年目の終わり、ユーザーごとに DB 1つ）**: 100 人 / 1,000 人で、**Workers + Turso Developer + R2 は $11.17 / $14.08**。D1 の見積もり $5.18 / $8.09 より **月 $5.99 高い**（Developer の基本料の分）。使用量はどちらの人数でも Developer の枠に収まる。Developer の枠を超えるのは、保存 9 GB（1人 1年 5 MB で延べ 1,800 人年）と、書き込み月 2,500 万行（約 5,000 人）あたり（本文からの計算）。
...
- **D1 で同じことをするなら**
  - D1 は1アカウント 50,000 DB（申請で数百万）で、ユーザーごとの DB を公式に想定している（本文で確認）。ただし Worker から DB を指すバインディングは設定で固定で、1スクリプトに約 5,000 個まで。実行時にバインディングを足す方法の記述は無い（本文を探したが記述なし）。REST API は主に管理用で、Cloudflare API 全体のレート制限（5 分に 1,200 回）がかかる（本文で確認）。
  - **ユーザーごとの SQLite なら Durable Objects のほうが素直**（本文からの読み取り）。数は無制限、1つ 10 GB、PITR は 30 日、行の料金は D1 と同じ。場所はヒント（`apac`・`apac-ne`）で指定でき、保証はない。
...
flowchart LR
  ...
  subgraph Plan["案（Turso、ユーザーごとの DB）"]
    A2[iPhone<br/>Embedded Replica / Turso Sync] -.->|同期 push・pull| T2[(Turso<br/>ユーザー A の DB)]
...
**比べる: Workers Paid + Durable Objects（ユーザーごとの SQLite）+ R2**（本文からの計算）
...
| ユーザーごとの DB を東京に置けるか | 東京のグループを1つ作り、全員の DB をそこに作れば、全員が東京になる。Developer はグループが1つだが、1つで足りる | 本文からの読み取り（TU19 のグループの制限と TU14 の作成 API から） | TU14、TU19 |
...
| プライバシーポリシーに書く値 | アカウント削除で DB ごと消すなら「削除後、最大5日」。DB を残して記録（行）だけ消すなら、PITR の保持期間（Developer 10 日）のあいだ過去の時点に残る | 本文からの読み取り（TU7、TU8 から） | TU7、TU8 |

D1（1つの DB に全員）と比べると、Time Travel は 30 日で縮められないため、行を消しても 30 日は残る（`server-platform.md`）。ユーザーごとの DB を丸ごと消せる形は、「即時削除」と「残る期間の短さ」で有利（本文からの読み取り）。
...
| 週ごとの見直しのやり方 | DB ごとに1本ずつ問い合わせる。管理・分析は、各ユーザーの DB から集計用の DB に書き出すなど、自分で集める | 本文からの読み取り（ATTACH の廃止から） | TU10 |
...
| 遅延 | Worker（日本の利用者なら日本の近く）から AWS 東京までの遅延の数値は無い。書き込みはコミットのたびに最大 50 ms（Developer）足される | 前半は本文を探したが記述なし、後半は本文で確認 | TU6 |
...
| nu-tori で渡すなら | 端末に渡すトークンは、ユーザーの DB だけ・読み取りだけ・期限つきにし、サーバーがサインインのあとに発行する。リポジトリとアプリには何も埋め込まない（ADR-0010 と両立）。ユーザーごとの DB なら、鍵を回して全トークンを無効にしても、影響はそのユーザーだけ | 本文からの読み取り | TU16、TU23 |
...
flowchart TD
  ...
  B --> X[ADR-0009 の「知識は1か所」とぶつかる]
  C --> Y[ADR-0009 の端末の例外は残る<br/>今のキャッシュと同じ課題]
```
...
## 8. D1 と Durable Objects で「ユーザーごとの DB」をするなら

| 問い | 答え | 確かさ | 出典 |
|---|---|---|---|
| D1 の DB の数 | 1アカウント 50,000 個（Workers Paid）。申請で「数百万〜数千万」に増やせる。D1 は「ユーザーごと・テナントごとの小さな DB を横に並べる」ことを想定し、DB の数には費用がかからない | 本文で確認 | CF1 |
...
| D1 で組むなら | ユーザーごとではなく、決まった数の DB に分ける（バインディングを固定で N 個持ち、ユーザー ID で振り分ける）形になる | 本文からの読み取り | CF1 |
| 1つの D1 に全員を入れたときの上限 | 1人 1年 5 MB で 10 GB は延べ 2,000 人年。100 人なら 20 年、1,000 人なら約 2 年 | 本文からの計算 | CF1 |
| Durable Objects（SQLite） | 数は無制限、1つ 10 GB、アカウント全体の保存は無制限。ID を名前から作る（`idFromName(userId)`）ので、ユーザーごとに実行時に選べる。行の料金は D1 と同じ（読み取り月 250 億、書き込み月 5,000 万を含む）。保存は 5 GB を含み、超過は $0.20/GB-月。リクエストは月 100 万回を含み、超過は $0.15/100 万。実行時間は月 40 万 GB-秒を含み、超過は $12.50/100 万 GB-秒 | 本文で確認 | CF7、CF8 |
...
**Turso-per-user が向く点**
- アカウント削除が DB を消す1回の呼び出しで済み、残るのは最大5日（D1 の Time Travel 30 日より短い）（TU8）
...
- 端末が DB に直接書く Turso Sync の形は、検証と計算を端末に持たせることになり、ADR-0009 とぶつかる（TU38）
