...
- **Linux でテストするロジックは値型に分ける。** 送信待ちの状態の進め方、やり直しの判断、日と時間帯の計算は SwiftData に触れない Swift パッケージに置き、SwiftData への写しはアプリ側に置く（「開発に使うエージェントツールと CI・配布」の決定と合う）。GRDB に替えても、Linux の対応が公式に保守されていないので、この分け方は変わらない。
