...
- 工程のゲートや引き継ぎなどの大きな確認は、新しい文脈のサブエージェントにその工程のスキルの観点と、下の「情報の置き場」の観点でレビューさせる。指摘ごとに、直さないとあとのセッションが何を間違えるかを確かめ、1行で言えるものだけ直し、ほかは見送る理由を書く。ストックの重複と、フローを今の決定の正本にしていることは、必ず直す。直したあとは、直した箇所が正しく反映されたかだけを確かめ、全体のレビューは回し直さない
