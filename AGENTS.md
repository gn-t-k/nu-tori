...
- 観測は3つに分ける。失敗の知らせとデバッグは Sentry（端末とサーバー）、ユーザーの行動とドメインの信号の分析は PostHog、要求ごとのログは Workers Logs。クラッシュは Apple の報告（Xcode Organizer、TestFlight）も見る。どれにも記録の中身（写真、文章、料理と材料の名前、発言の本文、体重・体脂肪率・kcal・栄養の値）を送らず、PostHog に送る数値は数・率・差・所要時間・旗だけにする。観測は申告を待たずに気づくためのもので、人の声は TestFlight のフィードバックと問い合わせで受ける。サーバーの側は `server/AGENTS.md` の「観測」、端末の側は `ios/AGENTS.md` の「観測」（ADR-0017）
