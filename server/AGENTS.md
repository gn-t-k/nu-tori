...
- 送り待ちを送る要求には、端末の送り待ちの件数、いちばん古い送り待ちの経過時間、写真の送り残しの数が添えられる（「観測」で使う）
...
- 提供元のワークスペースの月の支出の上限に達したときのエラー（HTTP 400 の `invalid_request_error`）は、アラームのやり直しに数えない。回数切れと同じく翌日に推定する予定に回す（写真の食事は料理 0 件のまま、名前を直したときと料理を足したときは今の料理のまま）。受け取ったときに数えた回数は戻さず、翌日に回した推定はもう一度数えない。上限は月が変わるまで解けないので、それまで毎日翌日に回る
...
- **Sentry**（`@sentry/cloudflare`、`@sentry/hono`）: Worker と Durable Object（アラームを含む）の例外と、対処した失敗（提供元のエラーなど）を送る。要求 ID をタグに、アカウント ID を `user.id` に付ける。SDK の v11 は、IP などのユーザーの情報、Cookie、HTTP のヘッダーと本文、URL のクエリ、DB の問い合わせのデータ、生成 AI の入出力を既定で集めるので、`dataCollection` ですべて切る。開発用の環境も送り、`environment` で分ける。メールの通知は、本番の新しい issue、急増、回帰にかける
...
- **保持とアカウントの削除**: PostHog の出来事は、消さなければ1年残る（無料のプラン）。アカウントの削除のときは、Durable Object を消して推定のアラームが止まってから、人と出来事を消す API で消す（消えるのは非同期で、週末になりうる。API より前に取り込んだ出来事だけが消えるため、この順にする）。Sentry は1人の分を消す手段が無いので、保持期間で消えるのに任せる（30 日。組織を作ってから 14 日の試用のあいだに届いた分は 90 日、バックアップは 90 日）
