...
- 送り待ちを送る要求には、端末の送り待ちの件数、いちばん古い送り待ちの経過時間、写真の送り残しの数が添えられる（「観測」の同期）
...
- 提供元のワークスペースの月の支出の上限に達したときのエラー（HTTP 400 の `invalid_request_error`）は、アラームのやり直しに数えない。回数切れと同じく料理 0 件のまま翌日に推定する予定に回し、ユーザーの推定の回数には数えない
- 一段で栄養まで返す経路を、評価用に持つ。アプリからは呼ばない。評価は開発用の環境で、正解のある写真の組（`docs/ui-design/0001-first-release/index.md` の「出荷後の検証」）だけを両方の経路に流して行い、ユーザーの写真は使わない
...
- **Sentry**（`@sentry/cloudflare`、`@sentry/hono`）: Worker と Durable Object（アラームを含む）の例外と、対処した失敗（提供元のエラーなど）を送る。要求 ID をタグに、アカウント ID を `user.id` に付ける。SDK の v11 は HTTP の本文、IP、生成 AI の入出力を既定で集めるので、`dataCollection` で切る。開発用の環境も送り、`environment` で分ける。メールの通知は、本番の新しい issue、急増、回帰にかける
- **PostHog**: 本番のサーバーだけが送る。`distinct_id` はアカウント ID。出来事は次のとおり
  ...
  - 量の修正: 量の出どころが推定の料理・材料を直す書き込みを当てるとき、置き換える前に、直した量 ÷ 推定の量、料理か材料か、推定のきっかけ、材料の栄養の出どころを送る
  - 見直しと、目標を立てた・切り替えたとき（見送った週も）: 見送ったかとその理由、当たった上限と下限、主な理由、推定消費量の変化（kcal の差）、推定消費量の出どころ。始まりから 4 週後と 8 週後の見直しでは、推定消費量と初期値の差も送る
  - 同期: 受け付けなかった書き込み（種類と理由）、写真の受け取りの失敗、要求に添えられた送り待ちの件数・いちばん古い送り待ちの経過時間・写真の送り残しの数
...
- **アカウントの削除**: PostHog は、人と出来事を消す API を受け口の Worker から呼ぶ（消えるのは非同期で、週末になりうる）。Sentry は1人の分を消す手段が無いので、保持期間（30 日）で消えるのに任せる。Workers Logs は 7 日で消える
