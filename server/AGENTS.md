...
- 提供元のワークスペースの月の支出の上限に達したときのエラー（HTTP 400 の `invalid_request_error`）は、アラームのやり直しに数えない。回数切れと同じく料理 0 件のまま翌日に推定する予定に回す。受け取ったときに数えた回数は戻さず、翌日に回した推定はもう一度数えない。上限は月が変わるまで解けないので、それまで毎日翌日に回る
...
- Durable Object の中では `waitUntil` が効かないので、送信は `await` する。送信の失敗は握りつぶし、推定や書き込みの適用を失敗させない（アラームのやり直しで推定を二重に回さないため）
- **置き場と、送る前の設定**: Sentry と PostHog はどちらも EU に置く（あとから移せない）。最初に送る前に、PostHog の規約5条の製品とモデルの開発への利用と、Sentry の Service Data Usage を断り、Sentry の DPA を受け入れる
- **保持とアカウントの削除**: PostHog の出来事は、消さなければ1年残る。アカウントの削除のときは人と出来事を消す API で消す（消えるのは非同期で、週末になりうる）。Sentry は1人の分を消す手段が無いので、保持期間で消えるのに任せる（30 日。組織を作ってから 14 日の試用のあいだに届いた分は 90 日、バックアップは 90 日）。Workers Logs は 7 日で消える
- **費用の知らせ**: Anthropic のワークスペースの通知（月の上限の 80%）、Cloudflare の予算アラート、PostHog の請求の上限のメール。Sentry は無料の枠を超えた分を捨てるので、知らせは要らない。エラー率の通知は Sentry に任せ、自前で作らない
