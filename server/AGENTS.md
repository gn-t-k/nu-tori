...
- ID は UUID。端末で作るものは端末が振る（振り方は `ios/AGENTS.md`）。サーバーが作るもの（推定が足す料理と材料、送った文章から作る食事、会話、AI の発言、サーバーで出す知らせ）は v4 で振る
...
- アカウントごとに変更の通し番号を振り、前回の続きからの変更を返す。消したものは削除の印を残して返す。サーバーが書いたもの（推定の結果と推定の状態、送った文章の読み分けの結果（作った食事か、入った会話）と応答の状態（作れなかった、回数切れ）、送った文章から作った食事、会話、AI の発言、サーバーで出す知らせ、いつもの時刻、1日の目安、推定消費量、体重の傾向、週ごとに残すもの）も同じ通し番号で返す
