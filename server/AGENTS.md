...
- LLM は Claude Sonnet 5 を使い、思考（reasoning）は明示して切る。保存するのは料理・材料・量の形だけにし、提供元の応答は残さない
- 材料は、名前、量と単位、1単位あたりの g、成分表の食品番号（成分表から引いたとき）、栄養の出どころ（栄養成分表示・成分表・AI の推定）、栄養の値を持つ。成分表と AI の推定は 100 g あたり、栄養成分表示は表示の単位あたりの値を、材料の量に換算する。栄養の値は材料に持たせて端末に届ける
...
- 成分表（日本食品標準成分表（八訂）増補2023年の本表）は、サーバーだけに置く。端末は成分表を持たない
...
- 推定の回数に、アカウントごとの1日の上限をかける。目安は 30 回で、守り方はまだ決めていない。数えるのは、写真・文章・名前を直したとき・料理を足したとき。「日」は `CONTEXT.md` の「日」で数える。超えた写真の食事は料理 0 件のまま保存し、翌日にアプリが閉じていても推定する。その回数は翌日の分として数える
