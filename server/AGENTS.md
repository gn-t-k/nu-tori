...
- HTTP の受け口は、記録を読み書きする要求なら、セッションを確かめ、回数の歯止め（「認証と不正利用防止」）をかけてから、その人の Durable Object を呼ぶだけにする。まだセッションのないサインインと Apple のサーバー間通知は、受け口の Worker の認証（「認証と不正利用防止」）で受ける。アカウントの削除（Apple のトークンの取り消し、D1・Durable Object・R2 の控えを消す）は、受け口の Worker でドメイン層を呼ぶ。Durable Object の中身を消すときも、その Durable Object の入口を呼んで行う
...
- **認証は Better Auth に任せる**。受け口の Worker に置き、Better Auth の表は D1 の中の認証の置き場に閉じる。ほかの表と Durable Object は、アカウント ID（Better Auth の user の ID。振り方は `generateId` で自分で決める）だけを見る
  - サインイン: 端末が Sign in with Apple で得た ID トークンと、端末が作った nonce を受け、Better Auth のネイティブの ID トークンの流れで確かめる
...
- **アカウントの削除**: Apple のリフレッシュトークンを取り消し、Durable Object・R2 の控え・テストユーザーを消し、最後に Better Auth の行（user・session・account）を消す。Better Auth の削除の前の確認（セッションの新しさ、`freshAge`）は切る
...
- アカウントごとの金額の上限は設けない。提供元の側では、環境ごとに Anthropic のワークスペースと API キーを分け、ワークスペースに月の支出の上限をかける（本番 $100、開発用 $10 から）。AI の発言も同じワークスペースで呼ぶ。AI の発言の回数は、推定の回数とは別に数える（数値と数える場所は「認証と不正利用防止」）。上限に当たったときの扱いも「認証と不正利用防止」
