...
- LLM は Claude Sonnet 5 を使い、思考（reasoning）は明示して切る。提供元の違いは、ドメイン層が定めた型の LLM の提供元の実装に閉じ込める。保存するのは料理・材料・量の形だけにし、提供元の応答は残さない
- 材料は、名前、量と単位、1単位あたりの g、成分表の食品番号（成分表から引いたとき）、栄養の出どころ（栄養成分表示・成分表・AI の推定）、栄養の値を持つ。成分表と AI の推定は 100 g あたり、栄養成分表示は表示の単位あたりの値を、材料の量に換算する。栄養の値を材料に持たせて端末に届けるので、端末は成分表を持たない
- 材料が持つ栄養は、成分表にあってヘルスケアに型のあるものすべて。AI の推定で出すのは主な栄養（エネルギー・たんぱく質・脂質・炭水化物・食物繊維・食塩相当量）だけにする。値の分からない栄養は「不明」にし、0 と区別する。AI の推定の主な栄養以外、栄養成分表示に書かれていない栄養、成分表の未測定（「-」）が「不明」になる
- 成分表（日本食品標準成分表（八訂）増補2023年の本表）は、サーバーだけに置く
- 量が 0 より大きいかといった値の妥当性は、ドメイン層で確かめる。Claude の構造化出力は、数値の `minimum`／`maximum` を使えない
- 料理の名前を直したときと、料理を足したときは、写真の控えと名前で①②を行う。名前と量を一緒に直したときは、量を固定して材料への割り振りだけを推定する。文章の食事は写真が無いので、名前だけを渡す
- LLM が「料理が写っていない」「見分けられない」と返したら、料理 0 件にする
- 食事を受け取った時点で保存して応答する。推定は裏で進めて、結果を料理・材料として保存する。提供元のエラーや時間切れは、裏で自動でやり直す
- 推定は、アカウントごとに1日 30 回までにする。数えるのは、写真・文章・名前を直したとき・料理を足したとき。「日」は `CONTEXT.md` の「日」で数える。超えた写真の食事は料理 0 件のまま保存し、翌日にアプリが閉じていても推定する
